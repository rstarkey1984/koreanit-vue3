...
### buil insert 를 쓰기 위해 items.map를 사용해 아래와 같이 변환함
```
items = [{},{}] -> values = [[],[]]
```
...
## 핵심 요약
- DB 연결은 createPool()로 서버 시작 시 한 번만 생성하고, 모든 코드에서 재사용한다.

- **대량 데이터는 bulk insert + ON DUPLICATE KEY UPDATE**로 중복 처리와 성능을 동시에 해결한다.

- 파일(fs) 기반 조회는 제거하고 컨트롤러 → 모델 → DB SELECT 구조로 완전히 전환한다.